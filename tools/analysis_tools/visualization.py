...
from projects.mmdet3d_plugin.core.bbox.structures.nuscenes_box import (
    CustomNuscenesBox,
    CustomDetectionBox,
    color_map,
)
...
def lidiar_render(
    sample_token, data, out_path=None, out_name=None, traj_use_perstep_offset=True
):
    bbox_gt_list = []
    bbox_pred_list = []
    ...
    visualize_sample(
        ...
        pred_annotations,
        savepath=out_path,
        traj_use_perstep_offset=traj_use_perstep_offset,
        pred_data=data,
    )
...
def get_gt_fut_trajs(nusc: NuScenes, anno, cs_record, pose_record, fut_ts) -> None:
    """
...
def get_gt_vec_maps(
    ...
    map_location = nusc.get(
        "log", nusc.get("scene", sample_rec["scene_token"])["log_token"]
    )["location"]
...
def visualize_sample(
    ...
    boxes_to_sensor(boxes_gt_global, pose_record, cs_record)
    ...
    for i, box in enumerate(boxes_est):
        ...
        if traj_use_perstep_offset:
            ...
            box.render_fut_trajs_grad_color(
                axes, linewidth=1, mode_idx=mode_idx, fut_ts=6, cmap="autumn"
            )
        else:
    ...
    line_segments = LineCollection(
        plan_vecs, colors=colors, linewidths=1, linestyles="solid", cmap=cmap
    )
...
if __name__ == "__main__":
    ...
    for id in tqdm(range(len(sample_token_list))):
        ...
        render_sample_data(
            sample_token_list[id], pred_data=bevformer_results, out_path=out_path
        )
        ...
        for cam in cams:
            ...
            if sensor_modality in ["lidar", "radar"]:
                ...
                boxes = [
                    ...
                    for record in bevformer_results["results"][sample_token]
                ]
                data_path, boxes_pred, camera_intrinsic = get_predicted_data(
                    sample_data_token, box_vis_level=BoxVisibility.ANY, pred_anns=boxes
                )
                ...
                if cam == "CAM_FRONT":
                    ...
                    s2l_r, s2l_t = obtain_sensor2top(
                        nusc, sample_data_token, l2e_t, l2e_r_mat, e2g_t, e2g_r_mat, cam
                    )
                ...
                lw = 6
                tf = max(lw - 3, 1)
                w, h = cv2.getTextSize(cam, 0, fontScale=lw / 6, thickness=tf)[
                    0
                ]  # text width, height
                # color=(0, 0, 0)
        ...
        pred_img = cv2.copyMakeBorder(
            pred_img, 10, 10, 10, 10, cv2.BORDER_CONSTANT, None, value=0
        )
        # font
        ...
        pred_img = cv2.putText(
            pred_img, "BEV", org, font, fontScale, color, thickness, cv2.LINE_AA
        )
        pred_img = cv2.putText(
            pred_img,
            plan_cmd_str,
